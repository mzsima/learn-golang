...
func (m MyCanvas) drawColorScale() {
	...
	d := math.Pi / 10.0
	ctx.Call("save")
	ctx.Call("translate", 150, 150)
	for i := 0; i < 20; i++ {
		ctx.Call("save")
		ctx.Call("rotate", d*float64(i))
		ctx.Call("translate", 100, 0)
		ctx.Set("fillStyle", fmt.Sprintf("hsl(%d, 90%%, 40%%)", i*18))
		ctx.Call("beginPath")
		ctx.Call("rect", -13, -13, 26, 26)
		ctx.Call("fill")
		ctx.Call("restore")
	}
...
func main() {
	...
	ctx.Call("fillText", "color wheel", 20, 20)
