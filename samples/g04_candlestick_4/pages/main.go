...
func drawPriceLabel(ctx js.Value, size Size, jsonData CandleStickResponse) {
	...
	tickRange = math.Floor(p/float64(cnt)) * float64(math.Pow10(cnt))

	lowerBound := tickRange * float64(int(min/int(tickRange)))
	upperBound := tickRange * float64(int(1+max/int(tickRange)))

	fmt.Println(lowerBound, upperBound, tickRange)
	for i := lowerBound; i <= upperBound; i += tickRange {
		ctx.Call("save")
		ctx.Call("translate", 8, (float64(max)-i)/yRatio)
		ctx.Call("fillText", fmt.Sprintf("%.0f", i), 0, 0)
		ctx.Call("restore")
	}
}
...
func main() {
	...
	h2.Set("innerHTML", "Chart04 WebAssembly! (made with GO)")
