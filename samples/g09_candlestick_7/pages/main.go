...
func main() {
	...
	h2.Set("innerHTML", "Chart07 WebAssembly! (made with GO)")
	...
	body.Set("onclick",
		js.FuncOf(func(this js.Value, args []js.Value) interface{} {
			...
			ctx.Call("save")
			ctx.Call("translate", x, y-50)
			ctx.Set("fillStyle", "#F2DF24")
			ctx.Set("strokeStyle", "#F29E55")
			ctx.Call("fillRect", 0, 0, 80, 52)
			ctx.Call("strokeRect", 0, 0, 80, 52)

			ctx.Set("fillStyle", "black")
			ctx.Call("fillText", fmt.Sprintf("x=%.0f, y=%.0f", x, y), 5, 20)
			ctx.Call("restore")
