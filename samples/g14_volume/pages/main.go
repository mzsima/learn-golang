...
func (c CandleStickResponse) Candles(size Size) (candles []Candlestick) {
	...
	for i, v := range c.Data {
		...
		volume := v.Volume / 2.
		path2.Call("rect", x-bar.w/2.0, 0, bar.w, volume)
		v.PathVolume = path2
...
func drawPriceLabel(ctx js.Value, size Size, jsonData CandleStickResponse) {
	...
	for i := lowerBound; i <= upperBound; i += tickRange {
		...
		ctx.Call("fillText", printer.Sprintf("%.0f", i), 30, 10)
		ctx.Call("restore")
	}
}

func drawChart(ctx js.Value, size Size, frameSize Size, candles []Candlestick, buttons []CurrencyButton, jsonData CandleStickResponse) {
...
func changeCurrency(ctx js.Value, canvas js.Value, v CurrencyButton, frameSize Size, size Size, buttons []CurrencyButton) {
	...
	go func() {
		...
		drawChart(ctx, size, frameSize, newCandles, buttons, newJsonData)
	}()
}
...
func main() {
	...
	h2.Set("innerHTML", "Chart12 WebAssembly! (made with GO)")
	...
	drawChart(ctx, size, frameSize, candles, buttons, jsonData)
